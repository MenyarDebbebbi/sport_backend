...
const register = async (req, res) => {
  try {
    ...
    try {
      const coaches = await User.find({ role: "coach", isActive: true }).select(
        "_id"
      );
      if (coaches.length > 0) {
            ...
            extra: user.pack
              ? {
                  pack: {
                    ...
                    level: user.pack.level,
                    price: user.pack.price,
                  },
                }
              : {},
          },
  ...
  try {
    ...
    if (!user) {
      ...
      return res.json({
        status: "success",
        message:
          "Si cet email existe dans notre base de données, un lien de récupération a été envoyé.",
      });
    }
    ...
    // Ici, vous devriez envoyer un email avec le lien de récupération
    // Pour l'instant, on simule l'envoi d'email
    console.log(
      `Email de récupération envoyé à ${email} avec le token: ${resetToken}`
    );
    ...
    res.status(500).json({
      ...
      error: "Erreur serveur",
      message:
        "Une erreur s'est produite lors de la récupération de mot de passe",
    });
  }
};
  ...
  try {
    ...
    if (error.name === "ValidationError") {
      return res.status(400).json({
        ...
        error: "Mot de passe invalide",
        message:
          "Le nouveau mot de passe doit contenir au moins 6 caractères avec au moins une minuscule, une majuscule et un chiffre.",
      });
    }
