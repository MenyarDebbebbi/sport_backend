  ...
  try {
    ...
    if (sessionId) {
      ...
      if (
        ...
        return res
          .status(403)
          .json({ error: "Vous n'êtes pas autorisé à modifier cette séance" });
      }
      const last = await CombinedWorkout.find({ sessionId })
        .sort({ order: -1 })
        .limit(1);
    ...
    try {
      if (
        Array.isArray(combinedWorkout.assignedTo) &&
        combinedWorkout.assignedTo.length > 0
      ) {
      ...
      console.error(
        "Erreur création notification (combined_workout):",
        notifyErr
      );
    }
    ...
    console.error(
      "Erreur lors de la création de l'entraînement combiné:",
      error
    );
    if (error.name === "ValidationError") {
      const errors = Object.values(error.errors).map((err) => ({
    ...
    return res.status(500).json({
      error: "Erreur serveur",
      message:
        "Une erreur s'est produite lors de la création de l'entraînement combiné",
    });
  }
};
...
const getWorkoutsBySession = async (req, res) => {
  try {
    ...
    const combinedWorkouts = await CombinedWorkout.find({
      sessionId,
      isActive: true,
    })
      .populate("createdBy", "firstName lastName")
      .populate(
        "workouts",
        "name type difficulty duration gifUrl sets repetitions rest weight"
      )
      .sort({ order: 1 })
      .skip(skip)
      .limit(parseInt(limit));
...
const getCombinedWorkoutById = async (req, res) => {
  try {
    const combinedWorkout = await CombinedWorkout.findById(req.params.id)
      ...
      .populate(
        "workouts",
        "name type difficulty duration sets repetitions rest"
      );
    ...
    if (
      ...
      !combinedWorkout.assignedTo.some(
    ...
    console.error(
      "Erreur lors de la récupération du programme combiné:",
      error
    );
    res.status(500).json({
      error: "Erreur serveur",
      message:
        "Une erreur s'est produite lors de la récupération du programme combiné",
    });
  }
};
...
const updateCombinedWorkout = async (req, res) => {
  try {
    ...
    if (
      existingCombinedWorkout.createdBy.toString() !==
    ...
    const updatedCombinedWorkout = await CombinedWorkout.findByIdAndUpdate(
      id,
      updateData,
      {
        new: true,
    ...
    try {
      ...
      console.error(
        "Erreur création notification (combined_workout_update):",
        notifyErr
      );
    }
    ...
    res.status(500).json({
      error: "Erreur serveur",
      message:
        "Une erreur s'est produite lors de la mise à jour du programme combiné",
    });
  }
};
...
const deleteCombinedWorkout = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(
        "Erreur création notification (combined_workout_delete):",
        notifyErr
      );
    }
    ...
    res.status(500).json({
      error: "Erreur serveur",
      message:
        "Une erreur s'est produite lors de la suppression du programme combiné",
    });
  }
};
...
const getUserCombinedWorkouts = async (req, res) => {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des programmes combinés:",
      error
    );
    res.status(500).json({
      error: "Erreur serveur",
      message:
        "Une erreur s'est produite lors de la récupération des programmes combinés",
    });
  }
};
