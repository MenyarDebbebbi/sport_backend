...
const updateMeal = async (req, res) => {
	try {
		const { name, description, type, items, imageUrl, assignedTo, tags, isActive } = req.body;
...
const reviewMeal = async (req, res) => {
	try {
		...
		res.json({
			message: `Repas ${status === 'approved' ? 'approuvé' : 'rejeté'} avec succès`,
			meal,
		});
	} catch (error) {
...
const getUserMealsForCoach = async (req, res) => {
	try {
		...
		console.error("Erreur lors de la récupération des repas de l'utilisateur:", error);
		res.status(500).json({
			error: 'Erreur serveur',
			message: "Une erreur s'est produite lors de la récupération des repas",
		});
