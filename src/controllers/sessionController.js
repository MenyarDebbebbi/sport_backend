...
const CombinedWorkouts = require('../models/CombinedWorkouts');

const getAllSessions = async (req, res) => {
	try {
		const { page = 1, limit = 10, type, difficulty, isPublic, search } = req.query;
		const skip = (page - 1) * limit;

		// Seules les séances publiques, créées par l'utilisateur ou assignées à l'utilisateur
		const filter = {
			isActive: true,
			$or: [{ isPublic: true }, { createdBy: req.user.id }, { assignedTo: req.user.id }],
		};
...
const createSession = async (req, res) => {
	try {
		const { name, description, duration, type, difficulty, isPublic, assignedTo, tags } = req.body;
...
const getSessionStats = async (req, res) => {
	try {
		...
		const avgDuration =
			sessions.reduce((acc, session) => acc + (session.duration || 0), 0) / totalSessions || 0;
...
const addExerciseToSession = async (req, res) => {
	try {
		...
		const nextOrder = existingExercises.length > 0 ? existingExercises[0].order + 1 : 1;
